...
**Codex 交代目标，Jev 跑完操作。让每一步点击都更轻。**

### ⚡ 决策快，操作连贯

Jev 官方报告的模型请求延迟为 **70–500 ms**。本项目在一次请求中同时决定“做什么”和“点哪里”，连续完成点击、滚动与切页，减少网络往返和主模型逐步介入的等待。

### 💰 高频操作，用低价模型完成

Jev 输入仅 **$0.042 / 百万 token，输出免费**。按每次计费 1 万输入 token 估算，**100 次决策约 $0.042**。把重复的网页决策交给 Jev，让 Codex 专注理解目标和回复结果。

### 🪶 少生成、少截图、少调用

搜索词等内容由 Codex 提前备好，Jev 选中后直接填入，省去额外的文字生成调用。默认读取页面文字与控件，省去逐步截图和图像理解的开销。

<sub>延迟与价格来自 [TypeSafe 官方](https://typesafe.ai/blog/introducing-system-one-models-and-jev)，核对于 2026-09-22。以上为模型指标与 API 费用估算；完整任务还包括 Codex、网页加载和重试的开销。</sub>
